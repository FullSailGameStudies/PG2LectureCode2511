	...
	do
	{
		...
		for (int i = 1; i <= n - 1; ++i)
		{
			...
			if (A[i - 1] > A[i])
			{
				...
				//swap(A, i - 1, i) 
				//FIRST WAY (OG swap)
				////temp variable to store one of the items
				//// type of temp must match the type of the items in the vector
				//std::string temp = A[i - 1];
				////copy the other item to the first items position
				//A[i - 1] = A[i];
				////copy the temp item to the second items position
				//A[i] = temp;
